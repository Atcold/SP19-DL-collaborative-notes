...
Unsupervised Learning can be seen with the same paradigm as Supervised learning: it learns a function from input-output pairs. However, instead of using labels as the targets - which in many cases are manually annotated and have a high cost to obtain at scale - Unsupervised  Learning aims at extracting information from the data itself without any annotation or supervision. This way a model can learn semantic feature representations of data, which can be further used in other tasks.

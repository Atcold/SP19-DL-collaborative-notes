...
Besides the practical issues for implementing backpropagation in application, a lot of methods have been proposed to improve training efficiency for deep neural networks. Example methods include pruning and weight sharing(Yann's paper), where ....

Ideas of pruning and weight sharing have been largely extended for the deep learning applications. Based on these methods, deep learning models can be compressed, achieving high efficiency in computation using less memory without loss of accuracy.  Researchers(eg Song Han) have implemented these methods and developed the compressed models like SqueezeNet(add reference here).  The basic techniques including pruning to reduce the number of connections between layers and then retrain the net. Cluster weights in the same layer and generate weights code book to reduce the memory usage. Apply Huffman encoding to save memory further(add reference here). Techniques to compress models help large deep learning models run smoothly on model devices and cloud. Researchers in this field are currently seeking general model design pipeline to let deep learning models suitable for different types of hardware.